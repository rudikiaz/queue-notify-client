...
function registerIpcHandlers(mainWindow, folderWatcher) {
  ...
  ipcMain.on('update-auto-start', (event, enabled) => {
    // Use the "--hidden" argument so the app launches minimized in the tray.
    app.setLoginItemSettings({
        openAtLogin: enabled,
        args: enabled ? ["--hidden"] : []
    });
    console.log("Auto-start behavior set to:", enabled);
  });
}
